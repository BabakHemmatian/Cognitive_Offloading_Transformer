...
#$ Sets path variable to the directory where this script is located
# NOTE: This is needed to ensure that the script can be run from any directory
dir_path = os.path.dirname(os.path.realpath(__file__))

## Set hyperparameters for training/testing
trial = 10 # so that we can run multiple trials of the same model with different hyperparameters. Change to a higher number to run a new trial. Set to "final" with training=False for testing evaluating the final model.
...
threshold = 0.36 # The confidence threshold for the rarest class. If the model's confidence in a class is below this value, it will not return that class. Only matters if thresholding=True and the value is greater than >.33 given the three main labels. 
epochs = 2 # Number of times the model weights are adjusted by going through the entire training set during training.
...
## Sets model path
model_path = dir_path+"\offloading_{}_{}".format(model_name,trial)
...
texts = {} 
labels = {}
with open("Offloading_Adjudicated.csv","r", encoding='utf-8',errors='ignore') as f:
    reader = csv.reader(f)
    for idx,line in enumerate(reader):
        if idx != 0:
            if line[10].strip() == "": # if the first two annotators agreed, use one of their labels
                label = title_label[line[9].strip()]
                if label != 3: # if the label is not "Other (write comment)", use it
                    texts[idx-1] = line[3].strip()
                    labels[idx-1] = label
            else: # if the third annotator did provide a label, use it
                label = title_label[line[10].strip()]
                if label != 3:
                    texts[idx-1] = line[3].strip()
                    labels[idx-1] = title_label[line[10].strip()]
...
print(Counter(final_labels)) # print the count of each label in the dataset
...
for cat in split_data:
    file_list.append(f"{split_data_path}\\text_{cat}.csv")
    file_list.append(f"{split_data_path}\\label_{cat}.txt")
...
# weights[2] = weights[2] * 1.1 # increase the weight of the "Heavy Offloading" class if needed to more strongly account for its rarity
...
else: # if using a trainer that penalizes when model predicts 2 but label is 1 (and/or vice versa)
    class WeightedTrainer(Trainer):
        def compute_loss(self, model, inputs, num_items_in_batch=None, return_outputs=False):
            labels = inputs.get("labels")
            outputs = model(**inputs)
            ...
            # Add confusion penalty: penalize when model predicts 2 but label is 1 (and/or vice versa)
            preds = torch.argmax(logits, dim=-1)
            confusion_penalty = 0.0

            # Penalize 0→2 and 2→0 mistakes more heavily
            confusion_mask_1_to_2 = (labels == 1) & (preds == 2)
            confusion_mask_2_to_1 = (labels == 2) & (preds == 1)
            
            penalty_weight = 1.0  # Adjust this (e.g., 0.5, 1.0, 2.0)
            penalty = penalty_weight * (confusion_mask_1_to_2.sum() + confusion_mask_2_to_1.sum()).float()
...
if training: # train a new model and save it
    ...
    print(f"Training a new classifier...")
    ...
    print(f"Loading a pretrained classifier...")
...
def get_prediction(text, threshold_class=2, threshold=threshold, use_thresholding=thresholding):
    ...
    # Tokenize the text
    # NOTE: Assuming 'tokenizer', 'max_length', and 'device' are defined globally.
    inputs = tokenizer(
        text,
        padding=True,
        truncation=True,
    ...
    # Model inference
    # NOTE: I am assuming the 'model' is defined globally.
    outputs = model(**inputs)
    ...
    if use_thresholding and probs[threshold_class] > threshold: # if using thresholds and the rare class passes the threshold
        return threshold_class # return the rare class
    else:
        if use_thresholding: # If thresholding was used and the threshold was not met, mask the thresholded class to prevent its selection if it's not the highest among others.
            masked_probs = probs.clone()
            masked_probs[threshold_class] = -1  # Prevent selection of the thresholded class
            return masked_probs.argmax().item()
        else: # If not using thresholding, or if threshold is not met, return the argmax of probabilities.
